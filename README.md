...
- Verified realistic demo-target path: `localhost:8089` -> demo-target ModSecurity/OWASP CRS -> `demo-portal` built from a separate checkout of this repository's `stable/cybertrace-target` branch -> demo-target audit log -> demo-target-bridge -> FastAPI internal WAF ingest
...
In the current repo, the application code, model-loading path, tests, dashboard shell, Supabase-backed runtime path, Docker smoke setup, and local WAF ingest proof are present. The dashboard browser path remains `Browser -> Next.js -> FastAPI`; the technical WAF proof path is `localhost:8088`; the realistic final demo WAF path is `localhost:8089`, with the separate `stable/cybertrace-target` checkout built as the `demo-portal` service.
...
- The realistic demo-target WAF path is published on `http://localhost:8089` when the `demo-target` profile is enabled; the profile also starts `demo-portal` from the separate `stable/cybertrace-target` checkout
