...
import { resendPasswordSetup } from '@/lib/server/db/account-management'
...
export async function POST(
  ...
  try {
    ...
    await resendPasswordSetup(session!.user.id, id)
    return NextResponse.json({ status: 'setup_sent' })
  } catch (error) {
