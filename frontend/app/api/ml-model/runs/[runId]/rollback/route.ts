...
import { rollbackRetrainingRun } from '@/lib/bff-client'
...
export async function POST(
  ...
  try {
    ...
    const parsed = RetrainingRollbackRequestSchema.safeParse(rawBody)
    if (!parsed.success) {
      return NextResponse.json(
        { error: { code: 'INVALID_REQUEST', message: 'Invalid rollback request.' } },
        { status: 400 }
      )
    }
    const result = await rollbackRetrainingRun(runId, parsed.data, {
      id: session.user.id,
