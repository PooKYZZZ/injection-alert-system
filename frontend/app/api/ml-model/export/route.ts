...
import { exportRetrainingSamples } from '@/lib/bff-client'
...
import { RetrainingExportRequestSchema } from '@/features/ml-model/schemas'
...
export async function POST(request: NextRequest): Promise<Response> {
  try {
    ...
    if (!parsed.success) {
      return NextResponse.json(
        { error: { code: 'INVALID_REQUEST', message: 'Invalid export request.' } },
        { status: 400 }
      )
    }
    const result = await exportRetrainingSamples({
      id: session.user.id,
