...
VALID_HTTP_METHODS = frozenset({
    ...
    "PROPPATCH", "UNLOCK", "SEARCH", "ACL", "BIND", "REBIND",
    "UNBIND", "DELETE", "PURGE", "LINK", "UNLINK", "VIEW",
})
...
def _extract_path_from_uri(uri: str, method: str = "GET") -> Optional[str]:
    ...
    is_authority_form = (
        method == "CONNECT" and 
        "://" not in uri and  # Not an absolute URL
        "/" not in uri  # No path component
    )
    ...
    if uri.startswith("http://") or uri.startswith("https://"):
        ...
        if "/" in rest:
            ...
            return "/" + path_part if path_part and not path_part.startswith("/") else (path_part if path_part else "/")
        else:
