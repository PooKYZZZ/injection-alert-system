...
| Runtime enforcement | Shadow-only implemented | `enforcement_recommendations` stores expiring policy intents after completed WAF triage; persistence is outside the single inference worker and expiry is anchored to the authoritative event timestamp; `/api/internal/enforcement/check` is dedicated-key protected, returns `ALLOW` for completed evaluations, and returns `503` when lookup is unavailable; the portal calls it server-side only for `/records/search`, logs degraded checks without secrets, and fails open; no real block/throttle/challenge is applied |
