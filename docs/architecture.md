...
| Runtime enforcement | Shadow-only implemented | `enforcement_recommendations` stores expiring policy intents from completed WAF triage; `/api/internal/enforcement/check` is dedicated-key protected and always returns `ALLOW`; the portal calls it server-side only for `/records/search`; no real block/throttle/challenge is applied |
