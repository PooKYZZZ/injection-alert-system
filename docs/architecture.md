...
| Real-time dashboard alerts | Implemented in code; local automated coverage | post-commit in-process broadcaster -> native FastAPI SSE -> authenticated Next.js streaming BFF -> one dashboard EventSource -> TanStack Query alert/stats invalidation; live no-refresh and named-tunnel proof remain to be executed |
