...
### 2026-08-26 — AUTH-012 / MFA-006 shared authentication surface pass

- **Status:** Implemented for the rendered route/component contracts; live MFA challenge lifecycle proof remains the explicit MFA-003 deferral.
- **Before → problem:** Authentication routes inherited a wide 760px framed surface with a separate header band, mixed heading scales, and inconsistent content widths. Sign In exposed a premature `Password required` footer; Forgot Password’s copy was indirect and treated non-OK responses as success; MFA’s code field was visually oversized and its normal state offered no obvious safe recovery path.
- **Research/principle:** OWASP authentication guidance supports generic recovery responses and non-enumerating error behavior; WAI-ARIA error guidance requires field-associated feedback; NIST/OWASP MFA guidance supports preserving recoverable challenge context while terminal expiry/lock states restart the transaction. The design decision was one compact auth surface with clear primary action, restrained secondary navigation, and no fabricated recovery mechanism.
- **Implementation:** `AuthShell.module.css` narrows the surface to 520px, removes the header band, reduces framing/shadow, and uses a compact responsive content inset. `authStyles.ts` adds the shared 28px heading class. Sign In places `Forgot password?` beside the password label and removes the premature footer text. Forgot Password now uses direct generic copy, preserves enumeration safety, and treats a non-OK response as a recoverable error. Reset/setup/email-verification, enrollment, recovery, and recent step-up headings use the same scale. `MfaVerifyForm` changes the progress text to sentence case, reduces the logical OTP field to 280px/44px, improves helper/disabled styling, preserves one-time-code/paste/autofill/numeric semantics and wrong-code selection/focus, keeps terminal restart handling, and adds a guarded `Use a recovery method` link to the existing protected recovery route.
- **Tests:** Focused auth suite **PASS**, 5 files / 19 tests; `npm run lint` **PASS**; `npm run typecheck` **PASS**; `git diff --check` **PASS**.
...
### 2026-08-26 — final current-branch audit after reopened critique pass

- **Status:** Complete for the local frontend remediation scope. The only meaningful P1 boundaries still open are the explicitly deferred `UMG-004` database invariant and `MFA-003` controlled live challenge harness; neither can be truthfully closed by frontend-only changes.
- **Current branch/worktree:** `codex/ml-health-admin-mfa-remediation` in `C:\Users\froi\.config\superpowers\worktrees\injection-alert-system\codex\ml-health-admin-mfa-remediation`. All implementation and remediation-document commits are present. The only remaining untracked path is the pre-existing user-owned `output/` directory; it was not inspected for mutation, staged, or overwritten.
- **Frontend validation:** `npm test -- --run --pool=threads` **PASS**, 100 files / 624 tests; `npm run lint` **PASS**; `npm run typecheck` **PASS**; `npm run build` **PASS** on Next.js 16.2.11 with all 43 application routes generated; `git diff --check` **PASS**. The full suite caught and drove the focused `aeab59b` fix that kept reusable confidence-bar precision intact while compacting only the Alerts table.
- **Browser validation:** The branch-specific frontend dev process was restarted after verifying its command path. A clean tab rendered `/login` at 1440×900 with no console warnings/errors and rendered `/forgot-password` at 1440×900 and 390×844. Keyboard focus was exercised on the sign-in form. Direct checks of `/dashboard`, `/alerts`, `/ml-health`, `/ml-model`, and `/user-management` correctly redirected to `/login` without a valid session. No authenticated ML Health, User Management, drawer, or live MFA-challenge screenshot is claimed for this pass because the available browser session had no safe authenticated challenge/session.
- **Runtime boundary:** `GET http://127.0.0.1:8000/health` returned HTTP 200, but the running backend process is from `G:\AI\PDDDD\injection-alert-system`, not this worktree. The frontend branch was therefore validated as a buildable/runnable BFF client, while branch-specific backend/model provenance is deliberately **not claimed** here. No backend code was changed in this remediation pass.
- **Console clarification:** The first reused browser tab retained two historical HMR parse-error entries from before the final frontend restart. The clean post-restart tab produced no warnings/errors, and the current source/build both parse `AlertDrawer.tsx` successfully; the stale entries are not current runtime failures.
- **Evidence files:** No new screenshot files or Playwright artifacts were created in this pass. Existing untracked `output/` evidence remains user-owned and untouched. The browser screenshots shown during validation were transient inspection output, not saved repository evidence paths.
- **Final finding state:** `SHELL-011`, `DASH-013`, `ALERT-014`, `MLH-014`, `UMG-011`, `AUTH-012`, and the implementation portion of `MFA-006` are resolved in source/tests. `RESP-004` is implemented in the responsive composition and component coverage, but authenticated visual confirmation remains unavailable. `DOC-006` is satisfied by this ledger. `UMG-004` and `MFA-003` remain explicit P1 deferrals. No unresolved in-scope P0 findings remain; no other P1/P2 issue is known beyond those two documented boundary deferrals and the authenticated visual-evidence limitation.

### 2026-08-26 — user-directed Dashboard KPI restoration

- **Status:** Implemented.
- **Before → problem:** The previous screenshot-remediation pass reduced the Dashboard summary from six KPI cards to four and moved average confidence plus the allowed non-Normal rate into a compact context row. It also omitted the ML Enforcement Map from the rendered dashboard. This removed operational context the user relies on for triage.
- **Decision/principle:** Restore the complete KPI summary and enforcement map while retaining the truthful no-traffic/unavailable states, the current BFF/data flow, the semantic metric wording, and the restrained page/header treatment from the remediation pass. Avoid duplicating the two restored metrics in a second context row.
- **Implementation:** `frontend/app/(dashboard)/dashboard/page.tsx` now renders six `StatCard` entries again: Non-Normal predictions, Blocked, Throttled, Allowed, Average model confidence, and Allowed non-Normal prediction rate (proxy). Progress-bar data is forwarded to the restored cards. The summary uses six columns at wide desktop widths, and the model-confidence section includes `MLEnforcementMap` backed by `non_normal_counts_by_confidence_tier`. The previous duplicate compact metric row was removed, and the four lower analytics panels can use four columns on wide screens. `frontend/app/(dashboard)/dashboard/page.test.tsx` asserts the six-card contract, proxy wording, enforcement-map data, and non-duplicated metric labels.
...
### 2026-08-26 — user-directed Alerts page restoration

- **Status:** Implemented.
- **Baseline:** The requested restore target was the pre-alert-remediation presentation at `c535d90`, while keeping the current BFF/API contracts, authentication/permission checks, mutation safeguards, and shared timestamp parsing used by ML Health and Model Operations.
- **Implementation:** `frontend/app/(dashboard)/alerts/page.tsx` returns to the compact route composition without the added page header. `FilterBar`, `AlertsTable`, and `AlertDrawer` return to the earlier framed filter bar, uppercase compact table headings, mobile horizontal-scroll guidance, full confidence label/precision, two-decimal CRS score, narrower 420px drawer, summary/core/evidence/workflow framing, and non-progressive training/system sections. `ActionLabel` and `TriageBadge` return to their earlier alert presentation; this also restores the same shared label treatment in Dashboard’s `RecentAlertsTable`. Alert evidence wording and relative-time spacing were restored to the same baseline semantics. The shared `StatusBadge` primitive remains because User Management still depends on it.
- **Preserved:** Browser-to-Next BFF-to-FastAPI flow, API payloads, role permissions, triage/action/label-review mutations, read-only viewer behavior, focusable controls, Radix dialog accessibility wiring, evidence relationships, deterministic administrative timestamps, and model/dashboard behavior outside the alert presentation were not removed.
