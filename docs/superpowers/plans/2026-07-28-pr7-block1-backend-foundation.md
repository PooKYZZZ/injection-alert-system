...
- [x] Switch/create `feat/pr7-critical-waf-enforcement` from the current T0 base.
- [x] Record current HEAD, migration head, SQLAlchemy/driver versions, recommendation model/transaction boundary, auth/error conventions, and PostgreSQL fixture availability without reading secrets.
- [x] Confirm worktree is clean and real Supabase is not used.
...
- [x] Run migration upgrade, downgrade, and re-upgrade against disposable/local PostgreSQL; confirm exactly one head.
- [x] Run the complete Block 1 suite, existing recommendation tests, PR5 tests, PR6 tests, affected backend tests, lint/type/static checks, `git diff --check`, and changed-file secret scan.
- [x] Review `git diff` for unrelated frontend/ML/runtime changes and confirm real Supabase was untouched.
- [x] Create clean commits on `feat/pr7-critical-waf-enforcement` only after all required gates pass.
- [x] Report the Block 1 backend foundation as complete while preserving the explicit Block 2/hosted/production gates below.

### Remaining gates outside this plan

Block 2 runtime activation, trusted hosted source identity, deployment rollout,
and production readiness remain intentionally out of scope. The controlled-local
snapshot API is disabled by default and does not authorize those gates.
