...
- PR4 shadow enforcement is opt-in (`ENFORCEMENT_MODE=shadow`): completed WAF triage can persist an expiring recommendation, while the dedicated internal check and portal `/records/search` path remain fail-open with `actual_decision=ALLOW`; hosted enablement is not claimed
