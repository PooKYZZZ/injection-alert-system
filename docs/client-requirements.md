...
| The system shall implement two-factor authentication (2FA). | Prefer a proven provider or TOTP implementation with enrollment, recovery, reset, and factor-change handling. | Implemented behind disabled rollout switches: factor-aware TOTP enrollment, login MFA, backup/email recovery, mandatory re-enrollment, retry-safe handoff, and recent-TOTP step-up. | Enable only after hosted migration, provider, browser, and pending-payload protection gates pass. |
...
| The system shall send email notifications after detection. | Use a transactional email provider/API; avoid building mail delivery infrastructure. Add duplicate-send protection and failure visibility. | The email-only outbox/worker boundary, supported templates, idempotency, deadlines, cancellation, lease reconciliation, and terminal scrubbing are implemented. Live provider delivery remains disabled and pending secret-bearing payload encryption is not yet approved. | Approve payload protection, configure/verify the provider, and run browser/provider deployment gates before enabling delivery. |
